...
/*   Updated: 2023/09/25 20:23:48 by ivanderw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	void	*image_ptr;
	...
	temp = mlx_xpm_file_to_image(m_game->mlx, xpm_file, &width, &height);
    //m_game->north_texture = mlx_xpm_file_to_image(m_game->mlx, xpm_file, &width, &height);
	if (temp == NULL)
	{
   	//if (m_game->north_texture == NULL) {
        return 1;
    } 
    int x_max = 1;
    int y_max = 1;
	...
	while (y < y_max)
	{
		...
		while (x < x_max)
		{
			mlx_put_image_to_window(m_game->mlx, m_game->mlx_win,  temp, x, y);
			x++;
     	}
		y++;
	}
